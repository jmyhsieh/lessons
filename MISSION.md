...
- 選用 Phase 11 任一工具卡後，能依問題使用 ccusage、QMD 或 Claude Code LSP：記錄一個工作階段的用量基準、搜尋一組核准的 Markdown 並回查原文，或唯讀找出一個程式符號的定義與兩個引用；需要更完整專案探索工作流時才另評估 Serena。
...
- 正式課程 Phase 1、3、4、5、6、9 採 beginner-first：每課只做一個小動作，先得到可見成果；Phase 2、7、8、10、11、12 是按需工具庫，實作頁不列入正式課程。
...
- Phase 5 入門 01–08 是獨立完成線；進階 09–12 只處理 Claude Design 簡報的可信對外交付，code-first 路線移到 open-slide 工具卡。
...
- 文件轉換是按需工具路線，不是 Phase 6 前置。一般文字型 PDF／Office 文件先試 MarkItDown；掃描 PDF、圖片或複雜版面才走 Docling；少量文件也可直接請 Claude Code 讀取。
...
- Design Engineering Skills 是按需工具路線，不是 Phase 4 的前置。安裝、元件審查、動態需求、按壓回饋、diff 驗收、候選盤點與改善計畫都依需求選用；任何 Skill 的建議都不取代 diff、測試、真實瀏覽器與人工判斷。
...
- 把 design system sync、完整 accessibility audit、claim ledger 或 proof pack 當成 Phase 4–6 初學者的結業條件；選用 open-slide 時只要求最小三頁 build 與實際開啟，選用 Playwright 時只要求一次受控 live check 或一個有紅綠證據的持久 test，不要求兩者都做，也不要求 Test Agents 或 CI。
