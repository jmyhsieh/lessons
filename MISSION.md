...
- 完成 Phase 13 入門 01–06 後，能判斷一項工作該走 Phase 3 或 OpenSpec，在真實程式庫初始化 OpenSpec、提出並審查一個小型變更，再以測試與程式差異完成實作、同步規格並封存。
- 選讀 Phase 13 進階 07–09 後，能逐份控制 proposal、delta specs、design 與 tasks 的產生順序，查出規格與程式的落差，並在同時處理多個 changes 時明確管理目前狀態與同步時機。
...
- Phase 4–13 採 beginner-first：每課只做一個小動作，先得到可見成果；進階術語、完整治理與 production-grade 驗證只能作為後續擴充，不得成為入門門檻。
...
- Phase 13 是獨立選修，不取代 Phase 3。需求追問、triage、除錯、架構診斷、TDD、code review 與 handoff 仍沿用 Phase 3；一旦選擇 OpenSpec，同一項變更就以 <code>openspec/changes/&lt;name&gt;/</code> 為規格正本，不再平行維護另一份 <code>.scratch</code> spec／tickets。
- Phase 13 入門只處理一個真實、小型 brownfield change，不要求回填整個程式庫，也不把 expanded profile、custom schemas、beta Stores 或多變更衝突處理變成前置。<code>/opsx:verify</code> 與 <code>openspec validate</code> 只能補充查核，不能取代 repo tests、程式碼審查與真人驗收。
...
- 把 OpenSpec 的 change artifacts 當成已實作證明、同時維護兩份規格正本，或在沒有真實變更需求時回填整個 brownfield codebase。
