...
- [Reference: Tools — Claude Code](https://code.claude.com/docs/en/tools-reference)
  內建 LSP tool 可查 definition、references、types、implementations 與 call hierarchy；需要對應語言的 code intelligence plugin 與 language server。Use for: Phase 11 第 03 課的優先符號探索路線。
- [Guide: Discover and install plugins — Claude Code](https://code.claude.com/docs/en/discover-plugins)
  官方 marketplace 的 code intelligence plugins 會啟用 Claude Code LSP；language server binary 仍需另外安裝。Use for: Phase 11 第 03 課的安裝前查核與停止線。
- [Repository: Serena](https://github.com/oraios/serena)
  透過語言伺服器提供符號與專案探索工作流；開源核心採 MIT License，JetBrains 付費外掛（plugin）是另一項產品。Use for: 內建 LSP 不合適或需要更完整專案探索時的 Phase 11 替代工具。
