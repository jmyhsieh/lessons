# Claude Code 工作技能改為獨立 Phase 9

Status: active

使用者要求把 Matt Pocock 的「Building Great Agent Skills: The Missing Manual」整理成獨立 Phase。Phase 9 不重複 Phase 2 的工具概覽或 Phase 3 的現成 workflow skills，而是從台灣工程師的角度，教 Claude Code 使用者把真實 repo 的重複流程寫成、審查並改善專案 Skill。

後續固定採入門 01–04、進階 05–09：先做出可執行 Skill、決定啟用方式、整理執行步驟／參考資料與完成條件；再學按需載入、引導詞、實際查找與驗證工作、精簡整理，以及新對話的啟用方式／執行結果測試。課文以繁體中文台灣工程用語為主，<code>SKILL.md</code>、欄位、指令與首次出現的原始術語保留英文。影片提供設計檢查架構，Claude Code 最新的檔頭設定、適用範圍與載入規則仍以 Anthropic 官方文件為準。
